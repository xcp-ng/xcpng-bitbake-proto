
PN = "rubygem-rspec-expectations"
PE = "0"
PV = "3.13.0"
PR = "3.el10"
PACKAGES = "rubygem-rspec-expectations rubygem-rspec-expectations-doc"


URI_rubygem-rspec-expectations = "https://vault.almalinux.org/10.0/CRB/x86_64_v2/os/Packages/rubygem-rspec-expectations-3.13.0-3.el10.noarch.rpm;unpack=0"
RDEPENDS:rubygem-rspec-expectations = "ruby(rubygems) ( ) rubygem(rspec-support) ( with  (REL)) rubygem(diff-lcs) ( with  (REL))"
RPROVIDES:rubygem-rspec-expectations = "rubygem(rspec-expectations) ( =  3.13.0) rubygem-rspec-expectations ( =  3.13.0-3.el10)"

URI_rubygem-rspec-expectations-doc = "https://vault.almalinux.org/10.0/devel/x86_64_v2/os/Packages/rubygem-rspec-expectations-doc-3.13.0-3.el10.noarch.rpm;unpack=0"
RDEPENDS:rubygem-rspec-expectations-doc = "rubygem-rspec-expectations ( =  3.13.0-3.el10)"
RPROVIDES:rubygem-rspec-expectations-doc = "rubygem-rspec-expectations-doc ( =  3.13.0-3.el10)"
