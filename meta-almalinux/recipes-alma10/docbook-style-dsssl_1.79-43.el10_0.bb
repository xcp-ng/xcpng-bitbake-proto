
PN = "docbook-style-dsssl"
PE = "0"
PV = "1.79"
PR = "43.el10_0"
PACKAGES = "docbook-style-dsssl"


URI_docbook-style-dsssl = "https://vault.almalinux.org/10.0/CRB/x86_64_v2/os/Packages/docbook-style-dsssl-1.79-43.el10_0.noarch.rpm;unpack=0"
RDEPENDS:docbook-style-dsssl = "/bin/sh ( ) sgml-common ( ) /usr/bin/perl ( ) perl(File::Basename) ( ) openjade ( ) perl(Getopt::Std) ( ) docbook-dtds ( )"
RPROVIDES:docbook-style-dsssl = "docbook-style-dsssl ( =  1.79-43.el10_0)"
