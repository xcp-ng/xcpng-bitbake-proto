
PN = "crash-gcore-command"
PE = "0"
PV = "1.6.4"
PR = "8.el10_0.1"
PACKAGES = "crash-gcore-command"


URI_crash-gcore-command = "https://vault.almalinux.org/10.0/AppStream/x86_64_v2/os/Packages/crash-gcore-command-1.6.4-8.el10_0.1.x86_64_v2.rpm;unpack=0"
RDEPENDS:crash-gcore-command = "rtld(GNU_HASH) ( ) libc.so.6(GLIBC_2.7)(64bit) ( ) crash ( >=  5.1.5)"
RPROVIDES:crash-gcore-command = "crash-gcore-command ( =  1.6.4-8.el10_0.1) crash-gcore-command(x86-64) ( =  1.6.4-8.el10_0.1)"
